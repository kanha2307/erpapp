...
const ProductCard = ({ product }) => {
  ...
  return (
    ...
    //   )}
    // </Card>
    <div className=" w-64 hover:shadow-lg cursor-pointer p-3 rounded-xl flex flex-col  bg-white">
      <img
        className="h-32 object-cover  rounded-2xl"
        src={`${uri}${product.image}` || "/path-to-placeholder.jpg"}
      />
      <div className="flex items-center mt-4 justify-between">
        <h1 className="font-[nats]  text-3xl">{product.name}</h1>
        <h1 className="font-[nats] text-xl text-[#00AC4F]">
      ...
      <div className="flex pl-4 mt-3 items-center justify-around">
        <Dropdown menu={{items}} placement="bottomLeft">
          <a onClick={(e) => e.preventDefault()}>
            <Space>
              <FiShoppingCart className="text-3xl pointer mr-5" />
            </Space>
          </a>
        </Dropdown>

        <button className="p-4 w-full rounded-full bg-[#FFBF00] hover:bg-yellow-600">
